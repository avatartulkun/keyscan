...
public interface OtpTokenDao {
    ...
    @Query("SELECT * FROM otp_tokens WHERE id = :id LIMIT 1")
    OtpToken findById(long id);
    @Query("SELECT * FROM otp_tokens WHERE itemId = :itemId LIMIT 1") OtpToken findByItemId(String itemId);

    @Query("SELECT * FROM otp_tokens WHERE id NOT IN (SELECT otpId FROM password_entries WHERE otpId IS NOT NULL AND id != :loginId) ORDER BY issuer, accountName")
    List<OtpToken> findAvailableForLogin(long loginId);
}
