...
public class HistoryAdapter extends RecyclerView.Adapter<HistoryAdapter.RecordViewHolder> {
    private static final int MAX_THUMBNAIL_BASE64_LENGTH = 4 * 1024 * 1024;
    private static final int MAX_THUMBNAIL_EDGE = 256;
    ...
    public void onBindViewHolder(@NonNull RecordViewHolder holder, int position) {
        ...
        holder.typeIcon.setImageResource("URL".equals(normalizeType(record.type)) ? R.drawable.ic_link : R.drawable.ic_text);
        holder.source.setText(holder.itemView.getContext().getString("GENERATE".equals(record.source) ? R.string.history_source_generated : R.string.history_source_scanned));
    ...
    public void submit(List<ScanRecord> newRecords) {
        records.clear();
        if (newRecords != null) {
            for (ScanRecord record : newRecords) {
    ...
    private String displayTitle(ScanRecord record) {
        String content = safeText(record.content);
        String title = record.title == null || record.title.trim().isEmpty() ? content : record.title;
        if (!maskSensitiveContent) return title;
        if ("URL".equals(normalizeType(record.type))) return title;
        if (record.title != null && !record.title.trim().isEmpty() && !record.title.equals(record.content)) return record.title;
        return "****";
    }

    private void bindThumbnail(RecordViewHolder holder, ScanRecord record) {
        holder.thumbnail.setImageDrawable(null);
        String encoded = record.thumbnailBase64;
        if (encoded == null || encoded.isEmpty() || encoded.length() > MAX_THUMBNAIL_BASE64_LENGTH) {
    ...
    private int sampleSize(int width, int height) {
        int sample = 1;
        while (width / sample > MAX_THUMBNAIL_EDGE || height / sample > MAX_THUMBNAIL_EDGE) {
            sample *= 2;
        }
        return sample;
    }
    ...
    private String groupLabel(long timestamp) {
        ...
        if (sameDay(item, today)) return context.getString(R.string.history_today);
        today.add(Calendar.DAY_OF_YEAR, -1);
        if (sameDay(item, today)) return context.getString(R.string.history_yesterday);
