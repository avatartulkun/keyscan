...
public final class OtpHelper {
    ...
    public static OtpToken parseUri(String raw) {
        return parseUri(null, raw);
    }

    public static OtpToken parseUri(Context context, String raw) {
        Uri uri = Uri.parse(raw);
        if (!"otpauth".equals(uri.getScheme())) throw new IllegalArgumentException(context == null ? "Invalid TOTP URI" : context.getString(R.string.otp_invalid_uri));
        ...
        token.createdAt = now;
        token.updatedAt = now;
        if (token.secret.isEmpty()) throw new IllegalArgumentException(context == null ? "Secret is required" : context.getString(R.string.otp_secret_required_error));
        return token;
    }
